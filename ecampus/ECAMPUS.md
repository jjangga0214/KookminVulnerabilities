...
테스트 결과, 다른 세션으로 동시에 동일한 계정에 로그인 해, 로그인 상태를 계속 유
...
만일 유지보수하기 불편한 구조 (그러나 SI 업체에게는 일단 계약을 완수하기 용이할
...
그럼에도 불구하고 유지보수 리뷰를 하지 않는다면 client 인 국민대학교에서는 자신
...
반영하고 끝내기 위해서 코드가 덕지덕지 붙고 유지보수성이 떨어져 가는지 견제하고
감독할 필요가 있다.
