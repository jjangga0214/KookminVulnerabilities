...
쿠키에서 `SameSite` flag는 비교적 최근의 속성이므로, 모든 브라우저에 구현되어 있는 것은 아니다. 그러나 구현되어 있는 브라우저는 외부 도메인의 사이트에서 request를 보낼 때, `SameSite` flag 가 붙은 쿠키를 전송하지 못하게 하여, MoodleSession 과 같이 민감한 쿠키 를 전송하지 않아 CSRF를 방지하는 데에 도움을 준다.   
